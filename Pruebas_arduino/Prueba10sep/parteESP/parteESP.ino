...
void loop() {
  ...
  if(stringReady){
    ...
    float timestamp = timeClient.getEpochTime();
    ...
    Firebase.setFloat("sensor2/"+dateTime+"/humidity", data["humidity"]);
    Firebase.setFloat("sensor2/"+dateTime+"/temperature", data["temperature"]);
    Firebase.setFloat("sensor2/"+dateTime+"/light/", data["light"]);
    Firebase.setFloat("sensor2/"+dateTime+"/timestamp/", timestamp);
